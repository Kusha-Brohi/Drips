...
// die();
    ...
    public function notification(){
       ...
       /// dd($data['gtpay_tranx_id']);
        $test =  $data['gtpay_tranx_id'];
       // dd($test);
        $result = explode("-", $test);
        //dd($result[1]);
    ...
    ////Ecc payment status///////////
     /// dd($data['gtpay_tranx_id']);
        $ecc_charges =  $data['gtpay_cust_id'];
      // dd($ecc_charges);
        $ecc_result = explode("-", $ecc_charges);
       // dd($ecc_result);
        $patient_id = trim($ecc_result[1],"DMS000:");
          // dd($patient_id);
	
	$ecc_ammount = DB::table('eccs')->first();
$price_of_ecc = $ecc_ammount->Ecc_price;
	
    if ($data['gtpay_tranx_amt'] == $price_of_ecc) {  
       DB::table('patients')->where('user_id', $patient_id)
...
if($data['gtpay_tranx_status_msg'] == 'Approved by Financial Institution'){
	//dd('test');
    DB::table('consultation')->where('id', $appoint_id)
