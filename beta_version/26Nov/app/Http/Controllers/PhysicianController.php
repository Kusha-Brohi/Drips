  ...
  public function PhysicianSignup(){
     ...
     public function Physician(){
      ...
      /*  $scheduled = DB::table('consultation')->where('doctor_id',Auth::user()->id)->where('Is_accept','pending')->get();*/
         $complain = DB::table('consultation')->where('booking_type','=','Scheduled')->where('doctor_id',Auth::user()->id)->where('status', 1)->get();
			//dd($complain);
    ...
    public function Consultation(){
        ...
        return redirect('/physician-dashboard/PhysicianLogin');
            ...
            $patient_data = DB::table('consultation')->where('id', $id)->where('doctor_id',Auth::user()->id)->first();
               // dd($patient_data);
...
/*               if ($request->hasFile('file')) {
 ...
 public function pendingConsultations(){
		...
		// $currentDate = date('Y-m-d');
       $complain = DB::table('consultation')->where('doctor_id',Auth::user()->id)->where('status', 1)->get();
        ...
        DB::table('zoom_meeting')->insert($insertArr);
                   ...
                   // Email code starts here
                       ...
                       Mail::send('mailingtemplates.zoomLink', ['data' => $data], function ($m) use ($data) {
                            $m->from('daltondeveloper.testing@gmail.com', 'DRIP');
                             $m->to($data['zoom_id_link'],'DRIP')->subject('DRIP');
                       });
