...
<div style=""> <!-- main Div -->
	...
	<div style="margin: 50px auto; color: #2a3342; border:7px solid #eee; ">
		...
		<h2 style="font-size: 16px; font-weight: inherit;">Dear <b>{{$emaildata['patientname']}}</b>! You have booked an appointment with  <b>Dr.{{$emaildata['name']}} </b> who is a specialist of {{$emaildata['speciality_0f_Doc']}}. on <b>{{$emaildata['date']}}</b> at <b>{{$emaildata['timing']}}</b>. The appointment type is <b>{{$emaildata['booking_type']}}</b>.</h2>
