...
public function submitProfile(Request $request){
		//dd($request);
		$user = DB::table('users')->where('id', Auth::user()->id)->first();
		$exp = Auth::user()->profile->expertise;
		$insertArr['name'] = $_POST['name'];
		$insertArr['email'] = $_POST['email'];
	...
	/*	$test = $_POST['expertise'];
		if($test != ''){
			$try = implode(",", $test);

			if($exp == ''){
				$insertArr['expertise'] = $try ;
			}
			else{
				$insertArr['expertise'] = $exp.",".$try ;
			}
	...
	/*		'name' => 'required',
		...
		if ($request->hasFile('Report')) {
          ...
          if(! is_null(request('images'))) {
			...
			$listing->where('customer_status','=','3');
			$Result = $listing->get();
        ...
        Session::flash('alert-class', 'alert-success'); 
        			return view('patient-dashboard.consultation',compact('listing','keyword','Result','photos'));
		}
