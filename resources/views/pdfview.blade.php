...
<div class="container">
	...
	<img src="{{asset('images/Drips-prescriptionTab.jpg')}}" style="width: 700px;">
...
<table style='position: absolute;margin-top: 400px; margin-left: -590px;'>
		<tr>
        	<th></th>  
       ...
       @endphp
		<tr>
        	<td>{{ $i }}</td>  
			<td>{{ucfirst($item->medication_name) }}</td>
			<td>{{ucfirst($item->medication_name2) }}</td>

			<!-- <td>{{ucfirst($item->frequency) }}</td> -->
			<td>{{ucfirst($item->duration) }}</td>
		</tr>
		@endforeach
	</table> 
</div>
