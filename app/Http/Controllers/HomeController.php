...
// die();
   ...
   DB::table('consultation')->where('id', $appoint_id)
    ...
    $appoint_type = $result[3];

    ////Ecc payment status///////////
       $ecc_charges =  $data['txnref'];
        $ecc_result = explode("-", $ecc_charges);
		$pat_ecc = $ecc_result[1];
        $pat_type = $ecc_result[2];
		$ecc_ammount = DB::table('eccs')->first();
		$price_of_ecc = $ecc_ammount->Ecc_price * 100;
	$patientData = DB::table('patients')->where('user_id',$pat_ecc)->first();
		if ($pat_type == "ecc_charge") {

      if($patientData->ecc_payment == 0){
         /*    dd('test');*/
            if($data['amount'] == $price_of_ecc) {  
            
            DB::table('patients')->where('user_id',$pat_ecc)
            ->update(['ecc_payment' => 1 ]);
					$eccemail = array();
					$eccemail['name'] = $patientData->fname;
					 $eccemail['patientemail'] = $patientData->email;
						$paideccamount = $price_of_ecc / 100;
					 $eccemail['amount'] = $paideccamount;
					//dd($eccemail['patientemail']);
                  
			    /*To Patient*/
                         Mail::send('mailingtemplates.eccConfirmation', ['eccemail' => $eccemail], function ($m) use ($eccemail) {
                           $m->from('contact@dripsmedical.com', 'DRIPS');
                           $m->to($eccemail['patientemail'],'DRIPS')->subject('DRIPS');
                      });
            return redirect('patient-dashboard/steps/'.$pat_ecc."?tabs=tab-step-131");  
     }
         }
            }
   ...
   DB::table('consultation')->where('id', $appoint_id)
    ...
    ///ajax call for appointment is different because we have two different authicate users/////
    ...
    public function Admin_call_ecc_popup(Request $request)
    {
       // dd($request->consultation_id);
        $consultation_id = $request->consultation_id;
        $patient_data = DB::table('consultation')->where('id', $consultation_id)->first();
        ...
        $inquiry->save();
             ...
             // Email code starts here
                        ...
                        Mail::send('mailingtemplates.confirmation', ['data' => $data], function ($m) use ($data) {
                                ...
                                $m->to($data['inquiries_email'],'DRIPS')->subject('DRIPS');
                        });
...
public function ECC_card(Request $request){
 ...
 public function patient_surgical_history_delete(Request $request){
    ...
    public function Unpaid(request $request){
        $data = DB::table('consultation')->where('id',$request->id)->first();
	    //	dd($data);
        $doc_data = DB::table('profiles')->where('user_id',$data->doctor_id)->first();
		 //$data = DB::table('consultation')->where('id',$request->id)->latest()->first();
    //  dd($data);
   //   $doc_data = DB::table('profiles')->where('user_id',$data->doctor_id)->first();
        return view('gtpayMerchant',compact('data','doc_data'));
    }
