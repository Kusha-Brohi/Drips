...
class UsersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $perPage = 300;
     if (!empty($keyword)) {
            $users = User::where('name', 'LIKE', "%$keyword%")->orWhere('email', 'LIKE', "%$keyword%")->orWhere('Is_deleted','=',0)
                ->orWhere('id', '!=', Auth::id())->paginate($perPage);
             
                
        } 
        else {
            $users = User::paginate($perPage)->where('id', '!=', Auth::id())->Where('Is_deleted','=',0);
        }
	...
	public function updatestatuscompleted($id, $con) {
		//dd($id,$con);
		$user= $id;
			if($con == "rejected"){
		$user = DB::table('users')->where('id', $id)->update(['is_approved' => 'rejected']);  
		
			}elseif($con == "Approved"){	
			$user = DB::table('users')->where('id', $id)->update(['is_approved' => 'Approved']); 
