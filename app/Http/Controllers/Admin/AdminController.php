...
$validArr['image1'] = 'required|mimes:jpeg,jpg,png,gif|required|max:10000';
}

$this->validate($request, $validArr);

$requestData = $request->all();
$imagetable = imagetable::where('table_name', 'portal_logo')->first();
...
$fileNameForm = str_replace(' ', '_', $fileNameExt);
$fileName = pathinfo($fileNameForm, PATHINFO_FILENAME);
$fileExt = $request->file('image1')->getClientOriginalExtension();
$fileNameToStore = $fileName.'_'.time().'.'.$fileExt;
...
imagetable::where('table_name', 'portal_logo')
->update(['img_path' => 'uploads/imagetable/'.$fileNameToStore]);
...
////////////////////portal logo end///////////////////////
	...
	public function newsletterInquiriesDelete($id) {
   	...
   	$consultation = DB::table('consultation')->where('is_deleted',0)->get();

     
        return view('admin.report.consultation',compact('consultation','pastAppointment','UpcomingAppointment','InProgress'));
    }
